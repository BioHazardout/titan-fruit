...
function LocaleSwitch({ path }: { path: string }) {
  ...
  return <div className="flex shrink-0 items-center gap-1 text-[10px] font-bold tracking-[.08em] xl:text-[11px]">{locales.map((locale) => {
...
export function Header({ path: providedPath }: { path?: string }) {
  ...
  return <header className="sticky top-0 z-50 isolate overflow-visible bg-[#021e10]/95 text-white lg:backdrop-blur">
    <div className="mx-auto flex h-[60px] w-full max-w-[92rem] items-center justify-between gap-3 px-4 sm:px-6 lg:h-[80px] lg:gap-4 lg:px-8 xl:gap-6 xl:px-10">
      ...
      <nav className="hidden min-w-0 flex-1 items-center justify-center gap-3 lg:flex xl:gap-4 2xl:gap-5" aria-label="Primary navigation">
        {nav.map((item) => item.children ? <div key={item.label} className="group relative">
          <button type="button" aria-haspopup="true" className={`inline-flex items-center gap-1 whitespace-nowrap text-[10px] font-semibold uppercase tracking-[.055em] transition-colors xl:text-[11px] 2xl:text-[11.5px] ${isTitanPage ? "text-lime" : "text-white/75 hover:text-lime"}`}>
      ...
      <div className="hidden items-center gap-3 lg:flex"><Link href={localizedPath(locale, "contact")!} className="whitespace-nowrap rounded-full bg-lime px-4 py-2.5 text-[10px] font-bold tracking-[.07em] text-forest transition-colors hover:bg-lime xl:px-5 xl:py-3 xl:text-[11px]">{locale === "es" ? "CONTACTO" : "CONTACT"}</Link><LocaleSwitch path={path} /></div>
