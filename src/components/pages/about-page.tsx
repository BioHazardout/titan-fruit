...
const highlights: Array<{ icon: IconName; value: string; label: string }> = [
  ...
  { icon: "package", value: "Own Packing Plant", label: "Technology and quality protocols" },
...
const benefits: Array<{ icon: IconName; title: string; text: string }> = [
  ...
  { icon: "globe", title: "Export expertise", text: "Deep knowledge of international export requirements." },
  { icon: "users", title: "Personal support", text: "Tailored support for every customer." },
];
  ...
  return <main>
    <section className="relative isolate h-[calc(100svh-60px)] min-h-[560px] overflow-hidden bg-forest text-white lg:h-[calc(100svh-80px)] lg:min-h-0">
      ...
      <div className="container relative flex h-full items-center"><div className="max-w-2xl">
        <h1 className="display text-5xl font-bold uppercase sm:text-6xl lg:text-7xl">About us</h1>
        <div className="mt-6 h-0.5 w-20 bg-lime" />
        <p className="mt-5 max-w-lg text-base leading-7 text-white/85">Our experience today lets us bring more Peruvian fruit to the world — fresh and frozen.</p>
      </div></div>
    </section>
    ...
    <section className="bg-[#f6f3ea] py-16 sm:py-24"><div className="container grid items-center gap-10 lg:grid-cols-[.82fr_1.18fr] lg:gap-16">
      <div className="max-w-xl"><p className="eyebrow text-leaf">Who we are</p><h2 className="display mt-4 text-4xl font-bold text-forest sm:text-5xl">We are Titan Fruit</h2><div className="mt-6 space-y-4 text-base leading-7 text-ink/70"><p>We&apos;re a Peruvian agro-export company and one of the country&apos;s first fresh dragon fruit exporters, active since 2023–2024. Today, we bring that experience to a broader range of fruit — avocado, blueberry, strawberry, and dragon fruit — across both fresh and frozen formats.</p><p>Beyond our own products, we offer packing and processing services for growers, traders, and exporters who need to prepare their fruits and vegetables for domestic or international markets. With a strategically located facility in Lima, we provide flexible processing capacity, efficient handling, and a reliable partner for businesses looking to expand their operations.</p></div><div className="mt-8 rounded-2xl border border-forest/10 bg-white/55 p-5"><p className="text-sm font-bold text-forest">From the field to your hands.</p><p className="mt-2 text-sm leading-6 text-ink/65">With our own crops and specialized packing operations, we maintain control over product quality from origin to final destination.</p></div></div>
    ...
    <section className="bg-white py-16 sm:py-24"><div className="container"><div className="mx-auto max-w-2xl text-center"><p className="eyebrow text-leaf">Why work with Titan Fruit?</p><h2 className="display mt-4 text-4xl font-bold text-forest sm:text-5xl">A trusted partner for your business.</h2></div><div className="mt-12 grid gap-7 sm:grid-cols-2 lg:grid-cols-6">{benefits.map((benefit) => <div key={benefit.title} className="text-center"><Icon name={benefit.icon} className="mx-auto h-9 w-9 text-leaf" /><h3 className="mt-4 text-xs font-bold uppercase tracking-[.06em] text-forest">{benefit.title}</h3><p className="mt-2 text-xs leading-5 text-ink/65">{benefit.text}</p></div>)}</div></div></section>

    <section className="bg-[#021e10] text-white"><div className="grid lg:grid-cols-[.34fr_.66fr]"><div className="relative min-h-[300px] overflow-hidden lg:min-h-[360px]"><Image src={images.nosotrosSection2} alt="Titan Fruit grower in the field" fill sizes="(max-width: 1024px) 100vw, 34vw" className="object-cover object-center" /></div><div className="flex items-center px-7 py-14 sm:px-12 sm:py-16 lg:px-[max(4rem,calc((100vw-76rem)/2))] lg:py-20"><div className="max-w-2xl"><p className="eyebrow text-lime">Sustainability</p><h2 className="display mt-4 text-4xl font-bold sm:text-5xl">Strong roots, a bright future.</h2><p className="mt-5 max-w-2xl text-base leading-7 text-white/80">We invest in our growers through long-term partnerships, ongoing training and support — helping ensure every piece of fruit reaches the world at its very best.</p><p className="mt-5 flex items-center gap-2 text-sm text-lime"><Icon name="leaf" className="h-5 w-5" />We believe in a healthier future through better food.</p></div></div></div></section>
  </main>;
}
